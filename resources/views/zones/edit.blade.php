...
@section('content')
    ...
    <form method="POST" action="{{ route('zones.update', $zone->id) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="title" value="{{ $zone->title }}" required>
        </div>
        <div class="form-group">
            <label>Status</label>
            <select name="status" class="form-control">
                <option value="1" {{ $zone->status == 1 ? 'selected' : '' }}>Publish</option>
                <option value="0" {{ $zone->status == 0 ? 'selected' : '' }}>UnPublish</option>
            </select>
        </div>
        <div class="form-group">
            <label>Coordinates</label>
            <textarea name="coordinates" id="coordinates" class="form-control" readonly>{{ $zone->alias }}</textarea>
        </div>
